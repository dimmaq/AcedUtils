...
//   AcedGridFrame 1.03                            //
//                                                 //
//   Фрейм для просмотра, поиска и корректировки   //
...
{ Фрейм TGridFrame отображает данные в виде таблицы из нескольких колонок
  с возможностью поиска в этой таблице подстрок, вводимых пользователем
  с клавиатуры. Текст в столбцах может выравниваться по левому краю, по правому
  краю или по центру. Если текст не помещается в ячейке, он может переноситься
  ...
  который зависит от состояния данной строки (обычная, выделенная, выделенная
  неактивная). Над таблицей располагается поле ввода. Текст, набираемый
  пользователем появляется в этом поле, одновременно он ищется по ячейкам
  таблицы. Если строка поиска начинается с символа '*', ищутся любые вхождения
  следующей далее подстроки в ячейках таблицы. Если этот символ отсутствует,
  ищутся ячейки, текст в которых начинается с введенной строки. Чтобы после
  нахождения строки продолжить ее поиск, надо нажать Ctrl+L. }
...
type
  ...
  { GetSelectedColumnIndex возвращает индекс выделенного столбца (или -1). }
  ...
  { Если True (по умолчанию), то выбранная запись выводится на темном фоне,
    когда фрейм не является активным элементом. }

    property InactiveSelection: Boolean read FInactiveSelection write FInactiveSelection;

  { Если True, то при выборе выделяется ячейка, а не строка (goRewSelect) }
